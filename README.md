...
_Most important are the following ones:_

- PARANOIA (Default: `1`)
  - Ranging from 1-4. Have a look at the section _'What are paranoia levels, and
    which level should I choose?'_ at [coreruleset.org/faq/](https://coreruleset.org/faq/).
...
- Audit Log (Default: JSON to stdout)
  - _see_ MODSEC_AUDIT_LOG
- ErrorLog (Default: JSON to stdout)
  - _see_ APACHE_ERRORLOG
- CustomLog (Default: JSON to stdout)
  - _see_ APACHE_ACCESSLOG
  - _see_ APACHE_PERFLOG
...
```sh
docker compose up -d
# navigate to http://localhost:8080 to verify it's up and runngin

# trigger modsec:
curl -X PUT "http://localhost:8080/status/500" -H  "accept: text/plain"
```
